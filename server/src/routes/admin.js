...
// Permission role names per user, aggregated so one row per user survives the
// role joins.
const Q_LIST_USERS = `SELECT au.id, au.email, au.display_name, au.is_active, au.last_login_at,
...
const Q_AUDIT_LOGS = `SELECT al.id, al.action, al.entity_type, al.entity_id, al.created_at,
