...
| 13 | `13_learning_module.sql` | free-form certifications; see the filtered-index note inside |
| 14 | `14_module_progress.sql` | `enrollment_module_progress` + `dbo.sync_enrollment_progress` **procedure** |

Data is **not** re-authored as T-SQL inserts. `02_seed`, `08_org_seed` and
`15_sample_course` are Postgres `INSERT`s; hand-porting them would be a large,
silent source of transcription errors. Load data by transferring it from a
populated Postgres instance instead.

Two places where the SQL Server tree is deliberately **not** a transcription,
both documented at the point of divergence:

- `13` replaces `UNIQUE(employee_id, certification_id, issued_date)` with a
  filtered unique index. Postgres treats NULLs in a unique index as distinct;
  SQL Server treats them as equal, so the constraint as written would allow each
  person exactly one self-reported certificate.
- `14` ships `sync_enrollment_progress` as a **procedure**, because a T-SQL
  scalar function cannot perform DML. `routes/learningModule.js` knows about the
  split and `EXEC`s it on this dialect.
